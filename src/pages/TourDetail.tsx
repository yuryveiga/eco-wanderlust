...
export function TourDetail() {
  ...
  return (
    <main className="min-h-screen bg-background font-sans overflow-x-hidden">
      ...
      {/* Mosaic Gallery Section - Soft Crop Optimization */}
      <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto mb-12">
        <div className="relative group overflow-hidden rounded-[2rem] shadow-xl bg-card border border-primary/5">
          <div className="grid grid-cols-1 md:grid-cols-6 md:grid-rows-2 gap-2 h-[500px] md:h-[600px] lg:h-[700px]">
            {/* Main Image - Now spanning more columns for better 3:2 aspect */}
            <div 
              className="md:col-span-4 md:row-span-2 relative overflow-hidden cursor-pointer group/item bg-muted/20"
            ...
            >
              <OptimizedImage 
                ...
                fit="cover"
                className="w-full h-full object-cover object-[center_30%] transition-transform duration-[length:2000ms] ease-out group-hover/item:scale-105" 
                fetchPriority="high"
              />
              <div className="absolute inset-0 bg-black/0 group-hover/item:bg-black/10 transition-all duration-500" />
            </div>

            {/* Sub-images - Now stacked in a 2-column sidebar for better proportions */}
            {images.slice(1, 5).map((img, idx) => (
              <div 
                key={idx}
                className="hidden md:block md:col-span-2 relative overflow-hidden cursor-pointer group/item bg-muted/10"
              ...
              >
                <OptimizedImage 
                  ...
                  containerClassName="w-full h-full"
                  fit="cover"
                  className="w-full h-full object-cover object-[center_30%] transition-transform duration-[length:2000ms] ease-out group-hover/item:scale-110" 
                />
                <div className="absolute inset-0 bg-black/0 group-hover/item:bg-black/10 transition-all duration-500" />
              </div>
            ))}
            ...
            {images.length < 5 && Array.from({ length: 5 - images.length }).map((_, i) => (
              <div key={`empty-${i}`} className="hidden md:block md:col-span-2 bg-muted/5 animate-pulse border border-white/5" />
            ))}
          </div>

          <Button 
            variant="secondary" 
            className="absolute bottom-8 right-8 gap-3 bg-white/90 backdrop-blur-xl hover:bg-white text-black font-black text-[11px] uppercase tracking-widest px-8 h-12 rounded-2xl shadow-2xl transition-all hover:scale-105 active:scale-95 z-30"
            onClick={() => openLightbox(0)}
          >
