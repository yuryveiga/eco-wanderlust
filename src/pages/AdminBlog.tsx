      ...
      <Dialog open={!!editing} onOpenChange={(open) => !open && setEditing(null)}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-6">
          <DialogHeader className="shrink-0 mb-4">
            <DialogTitle className="font-serif">{isNew ? "Criar Novo Post" : "Editar Post"}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="flex-1 flex flex-col gap-4 overflow-y-auto pr-2 pb-4">
              <div className="grid grid-cols-2 gap-4 shrink-0">
                <div className="space-y-2">
                  <Label className="font-sans">Título do Post</Label>
              ...
              <div className="space-y-2 shrink-0">
                ...
                <div className="flex items-center gap-4">
                  {editing.image_url && (
                    <img src={editing.image_url} alt="Capa" className="h-16 w-16 object-cover rounded shadow" />
                  )}
                  <div className="relative">
                    <Input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" id="blog-image-upload" disabled={isUploading} />
                    <Label htmlFor="blog-image-upload" className={`flex flex-col items-center justify-center h-16 w-32 border-2 border-dashed rounded-md cursor-pointer hover:bg-muted/50 transition-colors ${isUploading ? 'opacity-50' : ''}`}>
              ...
              <div className="flex justify-end gap-2 pt-4 bg-background shrink-0 pb-2">
                ...
                <Button onClick={handleSave} className="font-sans px-8 disabled:opacity-50" disabled={isUploading}>
